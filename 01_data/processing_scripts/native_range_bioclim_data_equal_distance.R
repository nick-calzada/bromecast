...
all_brte_samples <- read_csv('data/BRTEclim.csv', show_col_types = FALSE)
native <- all_brte_samples %>%
  filter(range == 'native') %>%
  left_join(read_csv('data/307tips.csv', show_col_types = FALSE), by = 'PopNum') %>%
...
cov.names <- read_csv("data/BioclimateOfOrigin_AllGenotypes.csv", show_col_types = FALSE) %>%
  colnames() %>% .[6:24]
...
# Save
# write_csv(grid_df, "may_27_native_range_grid_100x100_45_55_lat_0_22_lon_bioclim_data.csv")
